...
import { exactMatchCategories, exactMatchBrands, matchAnyInBrands, verifyCategoryPath } from "./sheetQuery";
...
- "stop at whatever level you've confidently reached, don't exclude" -> set stop_at_partial_level=true ONLY
  ...
  stop_at_partial_level=true on an entry where you also know the cat5 value.
    ...
    for (const call of sheetQueries) {
      const result =
        call.input.tab === "brands"
          ...
          : await exactMatchCategories(call.input.term);
      toolResults.push({
        type: "tool_result",
        tool_use_id: call.id,
