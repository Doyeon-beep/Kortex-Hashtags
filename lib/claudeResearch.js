...
import { exactMatchCategories, exactMatchBrands, verifyCategoryPath } from "./sheetQuery";
...
export async function researchNewEntity(segment, hashtagContext, budgetMs = RESEARCH_DEADLINE_MS) {
  ...
  for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
    ...
    for (const call of sheetQueries) {
      const result =
        call.input.tab === "brands" ? await exactMatchBrands(call.input.term) : await exactMatchCategories(call.input.term);
      toolResults.push({
        type: "tool_result",
        tool_use_id: call.id,
