...
**Don't treat a word as filler just because it feels short or generic.** Short, everyday-sounding words/phrases like "trip", "chat", "how to do", "at work", "party", "supply", "home", "gift", or "couple" frequently DO have their own standalone cat5 entry in the taxonomy. Don't assume something is too basic or too common to be tracked — always search the exact phrase in the sheet before deciding. (This is about checking whether an EXISTING entry already covers it - see the separate blocklist below under "New Category (cat5) Suggestion Rules" for words that should never be PROPOSED AS NEW even when no existing entry covers them.)
...
- **Never propose a new cat5 for any of these words/short phrases — too broad/generic to represent a distinct product or trend on their own, no matter how the search-volume check looks:** like, me, all, now, get, really, see, more, think, time, need, feel, first, over, well, to be, only, last, top, before, full, down, most, super, bye, start, add, almost, find, better, done, things, ready, after, wait, so much, without, stuff, for you, with me, run, life, to try, place, coming, stop, real, what's, later, open, works, step, check, name, less, drop, gone, hours, ask, play, outside, soon, hello, fix, change, spot, blender, total, making, instead, please, stronger, for all, remove, little. If one of these already exists in the sheet as an established cat5, using that existing match is fine - this is specifically about never CREATING a new one for these. If a segment resolves to one of these and nothing else fits, stop at whatever cat1-4 level you can confirm (stop_at_partial_level=true) rather than inventing a new cat5 for it.
