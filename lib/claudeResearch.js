...
import { exactMatchCategories, exactMatchBrands, matchAnyInCategories, matchAnyInBrands, verifyCategoryPath } from "./sheetQuery";
...
- "stop at whatever level you've confidently reached, don't exclude" -> set stop_at_partial_level=true ONLY
  ...
  stop_at_partial_level=true on an entry where you also know the cat5 value. Don't fill cat5 with the SAME
  name as one of cat1-cat4's own values just to have something in the cat5 slot (e.g. cat2="makeup" and then
  ALSO cat5="makeup" marked new) - that is not a new, more specific concept, it's the cat-level value itself
  duplicated. Real error: for a residual segment that was really just "makeup" (nothing more specific after
  the rest of the hashtag was already classified elsewhere), a new cat5 "makeup" was fabricated instead of
  setting stop_at_partial_level=true and leaving cat5 blank at cat2="makeup".
...
async function queryBrandsWithPeriodVariant(term) {
  ...
  if (!variant) return exactMatchBrands(term);
  return matchAnyInBrands([
    { column: "A", value: term },
...
// makeup" vs "pre shower makeup") independent of how the model happens to
// write the term it's checking - a real error was proposing "pre-shower
// makeup" as new when it already existed in the sheet, just spelled without
// the hyphen. Checked automatically in the same request rather than relying
// on the model to remember to re-query both forms itself.
...
async function queryCategoriesWithHyphenVariant(term) {
  const variant = withHyphenVariant(term);
  if (!variant) return exactMatchCategories(term);
  return matchAnyInCategories([
    { column: "E", value: term },
    { column: "E", value: variant },
  ]);
}
    ...
    for (const call of sheetQueries) {
      const result =
        call.input.tab === "brands"
          ...
          : await queryCategoriesWithHyphenVariant(call.input.term);
      toolResults.push({
        type: "tool_result",
        tool_use_id: call.id,
