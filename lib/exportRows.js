...
function researchEntryToRow(entry, hashtag, baseReasoning) {
  ...
  // For a bare failure/timeout/abort (no classification at all), the
  // technical detail (already logged server-side) is just noise in the
  // spreadsheet - show one clean flag instead of the raw error message with
  // "(needs human review)" appended a second time on top of it.
  const reasoning =
