...
// In-memory, best-effort cache of the FULL brands list, refreshed
// periodically — same "best effort, cuts down on repeat queries within a
// warm serverless instance" pattern as cat123Cache.js. Brands is a much
// smaller, more stable list than cat5 (which is deliberately never bulk-
// cached this way — see cat123Cache.js's own note about cat5's size and
// volatility), so caching it whole is safe, and lets a normalized/
// accent-insensitive fallback comparison run entirely in memory instead of
// yet another network round trip per hashtag.
let allBrandRowsCache = { rows: null, fetchedAt: 0 };
const BRAND_ROWS_TTL_MS = 60 * 60 * 1000; // 1 hour

async function getAllBrandRows() {
  const isFresh = allBrandRowsCache.rows && Date.now() - allBrandRowsCache.fetchedAt < BRAND_ROWS_TTL_MS;
  if (isFresh) return allBrandRowsCache.rows;

  const tab = process.env.BRANDS_TAB || "brands";
  const result = await runGvizQuery(tab, "select *");
  if (result.status !== "ok") {
    // Query failed — serve stale cache if we have one rather than treating
    // the brand list as empty.
    return allBrandRowsCache.rows || [];
  }
  allBrandRowsCache = { rows: result.rows, fetchedAt: Date.now() };
  return allBrandRowsCache.rows;
}

// Strips accents/diacritics and the punctuation a hashtag-derived query term
// can never contain (hashtags allow no periods, hyphens are handled
// separately, apostrophes/accents never appear at all), so a brand's real
// registered name can still match even when it's stylized. Real errors:
// "orebella" didn't match the brand tab's actual "Ôrəbella", and "cremerie"
// didn't match "Crémerie" — both got wrongly marked new="brand" even though
...
// Fallback used only after an exact (and period-variant) match already
// failed - compares the term against every cached brand name with accents
// and punctuation stripped from both sides. No extra network round trip
// beyond the periodic, cached full-list fetch above.
export async function fuzzyMatchBrand(term) {
  const rows = await getAllBrandRows();
  const target = normalizeForFuzzyMatch(term);
  if (!target) return { status: "ok", rows: [] };
  return { status: "ok", rows: rows.filter((row) => normalizeForFuzzyMatch(row[0] || "") === target) };
}
