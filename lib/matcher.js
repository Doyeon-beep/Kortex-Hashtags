...
async function matchAllFreeCandidates(segment) {
  ...
  for (let i = MIN_PART_LENGTH; i <= n - MIN_PART_LENGTH; i++) {
    ...
    combinedPhrases.push(`${left} ${right}`);
    // Also try simple pluralization of the trailing word - a hashtag like
    // "lowereyelash" (singular) should still find the sheet's "lower
    // eyelashes" (plural) for free, without needing AI research for
    // something this mechanical. All still folded into the same single OR'd
    // query below, so this doesn't add extra network round trips.
    if (!/s$/.test(right)) {
      combinedPhrases.push(`${left} ${right}s`);
      if (/[sxz]$|[cs]h$/.test(right)) {
        combinedPhrases.push(`${left} ${right}es`);
      }
    }
  }
