import { exactMatchCategories, exactMatchBrands, matchCategoryColumn } from "./sheetQuery";
...
// value (e.g. "lower eyelashes", "pink lemonade hard seltzer") only exists in
// the sheet WITH spaces, but arrives here as one unbroken string. Before ever
// trying to split the hashtag into multiple SEPARATE, unrelated concepts
// (trySegmentCompound below — which requires each half to independently
// exist on its own), check every point a space plausibly belongs and see if
// the sheet already has that literal combined phrase, per the guideline's
// "if the sheet already has a combined phrase, treat it as one segment" /
// "don't default to splitting into the smallest possible units" rules.
//
// Unlike trySegmentCompound's sequential search (which needs each half to
// independently match via several stem/abbreviation variants, so it checks
// one split point at a time and stops at the first hit), this only needs one
// exact-match query per candidate phrase, so all 2-way candidates are fired
// in a single parallel batch — even a long hashtag resolves in about one
// network round-trip instead of a growing sequential search. 3-way phrases
// (two inserted spaces) are only attempted if no 2-way phrase matched, since
// that combination count grows quadratically and most real combined phrases
// are 2-3 words.
async function queryPhraseCandidates(candidates) {
  if (candidates.length === 0) return null;
  const [catResults, brandResults] = await Promise.all([
    Promise.all(candidates.map((c) => exactMatchCategories(c))),
    Promise.all(candidates.map((c) => exactMatchBrands(c))),
  ]);

  for (let i = 0; i < candidates.length; i++) {
    if (catResults[i].status === "ok" && catResults[i].rows.length > 0) {
      return { phrase: candidates[i], matches: preferMainVertical(catResults[i].rows.map(rowToPath)), isBrand: false };
    }
  }
  for (let i = 0; i < candidates.length; i++) {
    if (brandResults[i].status === "ok" && brandResults[i].rows.length > 0) {
      return {
        phrase: candidates[i],
        matches: brandResults[i].rows.map(([brand, productLine]) => ({ brand, productLine })),
        isBrand: true,
      };
    }
  }
...
async function tryMatchCombinedPhrase(segment) {
  ...
  for (let i = MIN_PART_LENGTH; i <= n - MIN_PART_LENGTH; i++) {
    twoWay.push(`${segment.slice(0, i)} ${segment.slice(i)}`);
  }
  const found2 = await queryPhraseCandidates(twoWay);
  if (found2) return found2;

  if (n < MIN_PART_LENGTH * 3) return null;

  const threeWay = [];
  for (let i = MIN_PART_LENGTH; i <= n - MIN_PART_LENGTH * 2; i++) {
    for (let j = i + MIN_PART_LENGTH; j <= n - MIN_PART_LENGTH; j++) {
      threeWay.push(`${segment.slice(0, i)} ${segment.slice(i, j)} ${segment.slice(j)}`);
    }
  }
  return queryPhraseCandidates(threeWay);
}
