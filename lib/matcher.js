...
// tries the trailing word both pluralized AND singularized (the taxonomy's
// own convention is singular cat5 values - see the guideline's "Always
// singular" rule - but a hashtag is usually written plural, e.g.
// "skinsupplements"), for the same "free, no AI needed" reason as
// matchAllFreeCandidates's original version of this logic. Real errors:
// "skinsupplements"/"hairsupplements"/"acnesupplements" all failed to find
// the sheet's existing singular "skin supplement"/"hair supplement"/"acne
// supplement" because only the plural form was ever tried.
function spaceInsertedPhrases(text) {
  ...
  for (let i = MIN_PART_LENGTH; i <= n - MIN_PART_LENGTH; i++) {
    ...
    if (/ies$/.test(right) && right.length > MIN_PART_LENGTH + 2) {
      phrases.push(`${left} ${right.slice(0, -3)}y`);
    } else if (/es$/.test(right) && right.length > MIN_PART_LENGTH + 1) {
      phrases.push(`${left} ${right.slice(0, -2)}`);
    } else if (/s$/.test(right) && right.length > MIN_PART_LENGTH) {
      phrases.push(`${left} ${right.slice(0, -1)}`);
    }
    if (!/s$/.test(right)) {
