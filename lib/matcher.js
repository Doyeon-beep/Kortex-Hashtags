...
// Step 3.4's helper: checks whether the segment matches an existing cat4,
...
export async function matchSegment(rawSegment) {
  ...
  // Step 3.4: classification doesn't always need to reach cat5 — a bare word
  ...
  // at cat2 "nails"). This MUST run before the independent-word split below —
  // it originally ran after, which meant a whole word that's really one
  // coherent existing concept (e.g. "skincare") could get needlessly (and
  // wrongly) torn into two unrelated halves ("skin" + "care", each matching
  // something totally different) whenever that split happened to succeed
  // first, instead of being recognized as itself. A deeper existing cat5
  // match (checked above) is still always preferred over stopping early.
